...
export const mockBrainstormCards : BrainstormCard[] = [
    ...
    {
        id: 'card3',
        title : '엣지케이스 테스트용 아주 길고 긴 카드 제목을 넣어서 레이아웃이 안 깨지는지 확인하는 예시',
    ...
    if (searchQuery) {
        result = result.filter(
         (idea) =>
            ...
            idea.tags?.some((tag) => tag.includes(searchQuery))
        );
    }
...
) : Promise <{success : boolean}> => { //삭제했는지 성공 여부만 돌려줌
    await new Promise((r)=> setTimeout(r,500));
...
export const ConvertMockCardToIdea = async (
    ...
    return {
        ...
        title : card.title,
        summary : '이 아이디어는 할 일 관리를 위한 Todo 웹 앱입니다. 할 일 추가·삭제·완료 기능을 핵심으로 하며...',  // AI가 새로 생성한 요약 (mock이니 고정 텍스트)
        tags : card.tags,
